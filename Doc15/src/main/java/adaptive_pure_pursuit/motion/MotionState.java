package adaptive_pure_pursuit.motion;
...
public class MotionState {
    ...
    public double nextTimeAtPos(double pos) {
        if (Util.epsilonEquals(pos, this.pos, Util.kEpsilon)) {
            // Already at pos.
            return t;
        }
        if (Util.epsilonEquals(acc, 0.0, Util.kEpsilon)) {
            ...
            if (!Util.epsilonEquals(vel, 0.0, Util.kEpsilon) && Math.signum(delta_pos) == Math.signum(vel)) {
    ...
    public boolean equals(Object other) {
        return (other instanceof MotionState) && equals((MotionState) other, Util.kEpsilon);
    }
    ...
    public boolean coincident(MotionState other) {
        return coincident(other, Util.kEpsilon);
    }
