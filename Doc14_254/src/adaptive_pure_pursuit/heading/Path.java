...
public class Path {
    ...
    public TargetPointReport getTargetPoint(Translation robot, Lookahead lookahead) {
        ...
        /*
         * if (segments.size() > 1) { // Check next segment to see if it is closer. final Translation
         * next_segment_closest_point = segments.get(1).getClosestPoint(robot); final double
         * next_segment_closest_point_distance = new Translation(robot, next_segment_closest_point) .norm(); if
    ...
    public void checkSegmentDone(Translation robotPos) {
        ...
        if (remainingDist < PathSegment.kSegmentCompletionTolerance) {
    ...
    public void verifySpeeds() {
        ...
        for (int i = segments.size() - 1; i >= 0; i--) {
            ...
            maxStartSpeed += Math
                    .sqrt(maxStartSpeed * maxStartSpeed + 2 * PathSegment.kPathFollowingMaxAccel * segment.getLength());
        ...
        for (int i = 0; i < segments.size(); i++) {
            ...
            startState = new MotionState(0, 0, startState.vel(), startState.acc()); // the acc used to be startState.vel(), possible error
