...
public class AdaptivePurePursuitController {
    ...
    /**
     * Gives the RigidTransform.Delta that the robot should take to follow the path
     * 
     * @param pose
     *            robot pose
    ...
    public Command update(RigidTransform pose) {
        if (mReversed) {
            pose = new RigidTransform(pose.getRotation().rotateBy(new Rotation(Math.PI)), pose.getTranslation());
        }
    ...
    public static Translation getCenter(RigidTransform pose, Translation point) {
        ...
        final RigidTransform perpendicularBisector = new RigidTransform(normal, poseToPointHalfway);
        final RigidTransform normalFromPose = new RigidTransform(pose.getRotation().normal(), pose.getTranslation());
    ...
    public static double getLength(RigidTransform pose, Translation point, Translation center, double radius) {
        if (radius < kReallyBigNumber) {
            ...
            return radius * (behind ? 2.0 * Math.PI - Math.abs(angle.angle()) : Math.abs(angle.angle()));
        } else {
