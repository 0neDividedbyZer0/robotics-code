package adaptive_pure_pursuit.heading;

import java.util.Optional;

import adaptive_pure_pursuit.motion.MotionProfile;
import adaptive_pure_pursuit.motion.MotionProfileConstraints;
import adaptive_pure_pursuit.motion.MotionProfileGenerator;
import adaptive_pure_pursuit.motion.MotionProfileGoal;
import adaptive_pure_pursuit.motion.MotionState;
import geometry.Rotation;
import geometry.Translation;


/**
 * Class representing a segment of the robot's autonomous path. Needs the PathFollowingMaxAccel and the SegmentCompletionTolerance set for your robot
 */

public class PathSegment {
	public static final double kPathFollowingMaxAccel = 120.0; //in
	public static final double kSegmentCompletionTolerance = 0.1;  //in
    ...
    public double getLength() {
        if (isLine) {
            ...
            return deltaStart.norm() * Translation.getAngle(deltaStart, deltaEnd).angle();
        }
    }
    ...
    public Translation getPointByDistance(double dist) {
        ...
        if (isLine) {
            ...
            double deltaAngle = Translation.getAngle(deltaStart, deltaEnd).angle()
                    * ((Translation.cross(deltaStart, deltaEnd) >= 0) ? 1 : -1);
            deltaAngle *= dist / length;
            Translation t = deltaStart.rotateBy(new Rotation(deltaAngle));
    ...
    public double getRemainingDistance(Translation position) {
        if (isLine) {
            ...
            Translation deltaPosition = new Translation(center, position);
            double angle = Translation.getAngle(deltaEnd, deltaPosition).angle();
            double totalAngle = Translation.getAngle(deltaStart, deltaEnd).angle();
