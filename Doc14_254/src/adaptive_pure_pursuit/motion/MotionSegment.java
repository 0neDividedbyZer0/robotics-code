...
public class MotionSegment {
    ...
    public boolean isValid() {
        if (!Util.epsilonEquals(start().acc(), end().acc(), Util.kEpsilon)) {
            // Acceleration is not constant within the segment.
            System.err.println(
                    "Segment acceleration not constant! Start acc: " + start().acc() + ", End acc: " + end().acc());
            return false;
        }
        if (Math.signum(start().vel()) * Math.signum(end().vel()) < 0.0 && !Util.epsilonEquals(start().vel(), 0.0, Util.kEpsilon)
                && !Util.epsilonEquals(end().vel(), 0.0, Util.kEpsilon)) {
